...
//点击尺码
function sizeToRed(p,ssid){
	
	if($(p).attr("class")=='not-allow'){
		return;
	}
	
	$("#sizes a").each(function(){
	 	var clasx =  $(this).attr("class");
		if(clasx !='not-allow'){
	...
	<c:forEach items="${skus}" var="sku">
		if('${sku.colorId}' == colorId  && '${sku.size}' == ssid){
			$("#bbprice").html("￥"+'${sku.price}');
			skuId = '${sku.id}';
		}
	</c:forEach>
...
<div class="w ofc mt">
	<div class="l">
		<div class="showPro">
			<div class="big"><a id="showImg" class="cloud-zoom" href="${product.images[0]}" rel="adjustX:10,adjustY:-1"><img alt="" src="${product.images[0]}"></a></div>
		</div>
	</div>
		...
		<ul class="uls form">
			...
			<li><label>选择颜色：</label>
				...
				<c:forEach items="${colors}" var="color">
					<a onclick="colorToRed(this,'${color.id}')" href="javascript:void(0)" title="${color.name}" class="changToWhite"><img width="25" height="25" data-img="1" src="${product.images[0]}" alt="${color.name} "><i>${color.name}</i></a>
					</c:forEach>
				</div>
			</li>
...
<div class="r wr">
		...
		<div class="box bg_white ofc">
			<div id="detailTab1" class="detail">
				<img src="${product.description }" /><img src="/res/img/pic/p800a.jpg" /><img src="/res/img/pic/p800c.jpg" /><img src="/res/img/pic/p800d.jpg" />
			</div>
